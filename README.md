<img src="https://github.com/jallurivardhan/jallurivardhan/blob/main/linkedin%20banner.png" alt="Vardhan Banner" width="100%" />

# 👋 Hi, I’m Vardhan Jalluri

🎓 Master's in Computer Science | Data Analytics | SAP Basis Specialist  
📍 Currently based in the United States  
📅 Graduating: August 2025  
📫 jallurivardhan@gmail.com | [Portfolio](https://jallurivardhan.github.io) | [LinkedIn](https://www.linkedin.com/in/vardhanjalluri/)

---

## 🔧 About Me

- 💼 Former SAP Basis Consultant at **Accenture** with 2+ years of global project experience  
- 📊 Graduate Assistant at **MTSU**, improving system and student performance through data analytics  
- 🌩️ Skilled in **SAP Upgrades, AWS, Python, SQL, Tableau, Power BI**  
- 🧠 Passionate about building scalable, secure, and analytical solutions

---

## 🛠️ Technical Skills

- **Languages**: Python, SQL, Unix Shell  
- **Data Tools**: Tableau, Power BI, MySQL, MongoDB  
- **Cloud & Infra**: AWS, GCP, SAP HANA, SAP Solution Manager  
- **Others**: SAP Basis Administration, ActiveControl, ServiceNow, React.js, Node.js

---

## 📈 Experience Highlights

### 🧾 Graduate Assistant – International Affairs, MTSU  
- Analyzed recruitment data and improved document workflow  
- Supported immigration processes through SQL-based reporting

### 🏫 Student Technology Assistant – MTSU  
- Provided technical support for labs, boosting uptime and academic support  
- Scheduled professor calendars, managed student queries, and handled system updates
- 
### 🏢 SAP Basis Consultant – Accenture  
- Managed upgrades across SAP ECC, BI, CRM, and HANA environments  
- Configured 50+ HANA alerts and executed global system monitoring  
- Reduced release cycles by 25% through ActiveControl & ServiceNow  
- Resolved 250+ high-priority incidents across global regions in 6 months



---

## 🚀 Projects

- **🔁 Automated Ticketing Buddy** – ML-powered ticket routing (BERT, React)
- **🔐 Edu Secure Data Management** – Academic data encryption & centralization (MySQL, TDE)
- **📊 Interview Stress Analysis Dashboard** – React + MongoDB-based analytics
- **📉 Loan Approval Dashboard** – Increased efficiency by 20% using Power BI

---

## 📜 Certifications

- [Google Data Analytics](#)


---

## 📊 GitHub Stats

![Vardhan's GitHub Stats](https://github-readme-stats.vercel.app/api?username=jallurivardhan&show_icons=true&theme=default)

---

_If you'd like to collaborate on a project, learn more about my SAP or analytics work, or just chat about tech—feel free to reach out!_
